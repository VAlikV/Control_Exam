...
def plot_results(times: np.ndarray, positions: np.ndarray, velocities: np.ndarray, torque: np.ndarray, error: np.ndarray):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('logs/plots/Exam_robust_pos.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('logs/plots/Exam_robust_vel.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('logs/plots/Exam_robust_tor.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('logs/plots/Exam_robust_err.png')
    plt.close()
...
def joint_controller(q: np.ndarray, dq: np.ndarray, t: float) -> np.ndarray:
    ...
    lambda_1 = 1*np.array([400., 400., 400., 100., 100., 10.])

    s = (dq_d - dq) + lambda_1*(q_d - q)

    # v = ddq_d + Kp*(q_d - q) + Kd*(dq_d - dq) + calcVs(M_h, s)
...
def main():
    ...
    sim = Simulator(
        ...
        show_viewer=True,
        record_video=True,
        video_path="logs/videos/Exam_robust.mp4",
        fps=30,
        width=1920,
        height=1080
    )
...
if __name__ == "__main__":
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(times_1, s_1)
    plt.xlabel('Time [s]')
    plt.ylabel('Joint Positions Error [rad]')
    plt.legend()
    plt.grid(True)
    plt.savefig('logs/plots/Exam_robust_S.png')
    plt.close()